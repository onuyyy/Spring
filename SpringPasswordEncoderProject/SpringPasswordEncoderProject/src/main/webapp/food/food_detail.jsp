...
<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=2fbbfbecfa23ef7be284d3217f567a93&libraries=services"></script>
          ...
          <td>
            <table class="table" v-for="re in reply_data">
              ...
              <tr style="display:none" :id="'reply'+re.no" class="updates">
                <td colspan="2">
                  <textarea rows="3" cols="55" style="float: left" ref="msg" id="msg">{{re.msg}}</textarea>
	              <input type=button value="댓글수정"
	              style="height: 66px;background-color: blue;color:white" @click="replyUpdate(re.no)"> 
                </td>
              </tr>
            </table>
          </td>
   ...
   new Vue({
	 ...
	 methods:{
		 ...
		 addScript:function(){
			 ...
			 script.src='http://dapi.kakao.com/v2/maps/sdk.js?autoload=false&appkey=2fbbfbecfa23ef7be284d3217f567a93&libraries=services'
		 ...
		 replyUpdate(no){
			 ...
			 axios.post('../food/reply_update_vue.do',null,{
				params:{
					no:no,			// 매개변수 no
					msg:msg,		// replyUpdate 메서드 내에서 선언된 msg(this 쓰면 안 됨)
					fno:this.fno	// this. => 전역변수 사용시에만 쓴다
				} 
			 }).then(response=>{
				 console.log(response.data)
				 this.reply_data=response.data	// updated => 변경된 데이터 값으로 출력
				 $('#reply'+no).hide();
				 $('#up'+no).val("수정");
				 this.no=0;
			 }).catch(error=>{
