...
// repository : dao 메모리 할당 / component : 일반 클래스 메모리 할당

@Component("mc")		// id 안 정해주면 default 아이디가 생성된다 => mainClass
public class MainClass {
	...
	public static void main(String[] args) {
		...
		// getBean으로 가져올 건지, dao를 받아올 건지 선택
//		 ApplicationContext app=new ClassPathXmlApplicationContext("app2.xml");
		AnnotationConfigApplicationContext app=new AnnotationConfigApplicationContext(EmpConfig.class);
		 ...
		 MainClass mc=(MainClass)app.getBean("mc");
			...
			for(EmpVO vo:list) {
				System.out.println(
						vo.getEmpno()+" "+vo.getEname()+" "+vo.getJob()+" "+vo.getDbday()+" "+vo.getSal()+" "+vo.getDvo().getDname()+" "+vo.getDvo().getLoc()
						);
			}
